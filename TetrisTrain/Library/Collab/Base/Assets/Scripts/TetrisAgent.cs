...
{    
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if(gameObject.GetComponent<GameController>().getField().isRedrawed == true)
        {
            int lines = gameObject.GetComponent<GameController>().getField().linesMatched.Count;
            int height = 0;
            int holes = 0;
            int bumpiness = 0;
            ...
            {   
                ...
                {                
                    ...
                    else if(currentHeight > 0 && gameObject.GetComponent<GameController>().getField().grid[m, n] == 0)
                    {
                        holes = holes + 1;
                    }
                }
            ...
            Debug.Log(-0.51f*height + 0.76f*lines - 0.36f*holes - 0.18f*bumpiness);
            SetReward(-0.51f*height + 0.76f*lines - 0.36f*holes - 0.18f*bumpiness);
        }

        if (gameObject.GetComponent<GameController>().getField().scoreToAdd != 0)
        {
            SetReward(gameObject.GetComponent<GameController>().getField().scoreToAdd);
