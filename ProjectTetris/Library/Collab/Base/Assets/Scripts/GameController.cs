...
public class GameController : MonoBehaviour {
    ...
    void Update()
    {
        if (sharedData.getGameNotOver())
        {
            ...
            if (field.linesMatched.Count > 0)
            {
                ...
                audioSource.clip = field.linesMatched.Count >= 4 ? tetris : lineClear;
                audioSource.Play(); 
        ...
        if (player1 && SceneManager.GetActiveScene().buildIndex == 2)
        {
    ...
    {
        return (float) (field.score * (1 / Mathf.Log((GetComponent<TetrisAgent>().tetrisHeight + 2), 2)));
    }
